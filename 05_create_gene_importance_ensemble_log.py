...
file = open("./csvs/gene_importances/rnd_used.csv", "r")
...
for _ in range(3):
    RND_STATE_HYPER = random.randint(1, 4_000_000_000)
    ...
    try:
        df_non_zero.to_csv(
            "./csvs/gene_importances/genes.csv", mode="a", index=False, header=False
        )
    except:
        pass
    pd_rnd_used = pd.Series(rnd_used)
    pd_rnd_used.to_csv("./csvs/gene_importances/rnd_used.csv", index=False)
